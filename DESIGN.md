...
- **Narrative order:** every generated storefront opens with a two-to-three-scene hero slider, continues into one bounded two-to-three-chapter Story Scroll, and then reaches the product catalog. Pair each scene with one image and meaningful merchant or product copy; never turn the unused asset library into a long image pile or collage. The generated document owns this composition: its desktop hero fills the preview stage and never inherits compact sizing, insets, or split-media rules from a legacy store layout. Any additional signature experience belongs after the catalog and may not repeat the opening media without a clear editorial reason.
