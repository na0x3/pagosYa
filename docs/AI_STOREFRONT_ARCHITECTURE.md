...
## Dormant conversation experiment

The API retains store-scoped conversation and bounded-revision primitives as a reversible experiment, but the merchant dashboard does not expose a conversational storefront editor. The supported merchant flow is editor-first: open **Apariencia**, manipulate the canonical preview directly, or opt into a batch of three private visual directions and explicitly apply one.

Dormant conversation records do not trigger model calls or cost unless those endpoints are invoked. If this experiment is reintroduced, it must remain behind an explicit feature flag, use only allow-listed visual transformations, and never become the default dashboard or apply or publish a proposal automatically.
