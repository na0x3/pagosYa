...
The homepage has an explicit action hierarchy. Inicio is an introduction and decision surface, not a directory of every useful destination: a fresh composition gets one dominant filled action, normally the primary purchase or booking action, and at most two quieter secondary destinations near the opening. Story, location, contact, gallery, promotion, and FAQ content with enough substance for its own destination should become a real page linked from the shared header/footer. Buttons are reserved for immediate actions or state changes; navigation uses anchors. This keeps secondary information discoverable without making the opening feel like a control panel.
...
The shared section capability registry is the renderer contract for hero, story, catalog, gallery, contact, location, and links. It declares compatible families, layouts and motions, media bounds, and named slots such as `heading`, `actions`, `primary-media`, and `products`. Generation is validated against that registry. Event tickets are retired; old event sections are discarded when rendering saved storefronts.
