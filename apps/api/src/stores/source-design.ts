...
Style the runtime as carefully as the homepage using its exact hooks: [data-pagosya-product] .product-detail__layout, .product-detail__gallery, .product-detail__photo, .product-detail__copy, .product-detail__pricing, .product-detail__options fieldset/legend, .product-detail__values, [data-product-option][aria-pressed=true], .product-detail__swatch, .product-detail__purchase, .product-detail__quantity, .product-detail__subtotal, .product-detail__buy, [data-option-price], .product-detail__assurances and .product-detail__sticky. Do not guess classes such as .product-detail__option. Never hide option prices, totals or assurances; the runtime breadcrumb replaces back links. Keep choices near the title. Preserve the empty data-pagosya-product-page mount and let the runtime render the selected product; never place a second static product configurator beside it. On local edits apply this guidance only to the requested scope.`;
