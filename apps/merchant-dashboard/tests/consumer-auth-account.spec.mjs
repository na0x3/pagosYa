...
test("consumer can use Google and then delete access with retained-evidence disclosure", async ({ page }) => {
  ...
  await expect(page.locator('[data-market-mode="events"], [data-target="faceEntry"], #faceEntry')).toHaveCount(0);
  expect(requests.some(request => request.path.startsWith('/events'))).toBe(false);
