...
for (const mobile of [false, true]) test(`create product form saves directly and retains failed uploads (${mobile ? 'mobile' : 'desktop'})`, async ({ page }) => {
  ...
  let creates = 0, uploads = 0, aiMessages = 0, catalogReads = 0, suggestions = 0;
  ...
  await page.route('**/api/v1/**', async route => {
    ...
    if (path.endsWith('/stores')) body = [{ id: 's1', name: 'Tienda', slug: 'tienda' }];
    ...
    else if (path.endsWith('/product-highlights/suggestions')) {
      suggestions++;
      expect(route.request().postDataJSON()).toMatchObject({ name: 'Hamburguesa de la casa', description: 'Carne 150 g, queso y pan artesanal.' });
      body = { highlights: [{ icon: 'flame', label: 'A la parrilla', detail: 'Carne 150 g' }, { icon: 'wheat', label: 'Pan artesanal' }] };
    }
      ...
      expect(route.request().postDataJSON()).toMatchObject({ highlights: [{ icon: 'flame', label: 'A la parrilla', detail: 'Carne 150 g de res' }] });
      expect(route.request().postDataJSON()).toMatchObject({ name: 'Hamburguesa de la casa', description: 'Carne 150 g, queso y pan artesanal.', amount: 3550, currency: 'BOB', stock: 12, imageUrls: ['/v1/uploads/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa.webp'] });
  ...
  await expect(dialog.getByText('Paso 1 de 8')).toBeVisible();
  ...
  await expect(dialog.getByText('Paso 2 de 8')).toBeVisible();
  ...
  await dialog.getByRole('button', { name: 'Siguiente' }).click();
  // YAPI proposes highlights once; the owner edits them and keeps only what they approve.
  await expect(dialog.getByText('Paso 4 de 8')).toBeVisible();
  await expect.poll(() => suggestions).toBe(1);
  await expect(dialog.locator('[data-highlight-row]')).toHaveCount(2);
  await expect(dialog.locator('[data-highlight-row] [data-highlight-text]').first()).toHaveValue('A la parrilla');
  await dialog.locator('[data-highlight-row] [data-highlight-detail]').first().fill('Carne 150 g de res');
  await dialog.getByRole('button', { name: /^Quitar destacado Pan artesanal/ }).click();
  await expect(dialog.locator('[data-highlight-row]')).toHaveCount(1);
  await dialog.getByRole('button', { name: 'Siguiente' }).click();
  ...
  await expect(dialog.locator('[data-guide-step="5"] figure img')).toHaveCount(1);
  await dialog.getByRole('button', { name: 'Guardar producto', exact: true }).click();
...
test('the product guide saves a specification sheet from Nombre: valor lines', async ({ page }) => {
  ...
  for (let step = 3; step < 8; step++) await dialog.getByRole('button', { name: 'Siguiente' }).click();
  await dialog.getByRole('button', { name: 'Guardar producto', exact: true }).click();
  await expect(page.getByRole('heading', { name: 'Producto creado', exact: true })).toBeVisible();
...
test('YAPI creates a styled scene from the uploaded product photo only after the merchant reviews it', async ({ page }) => {
  ...
  await dialog.getByRole('button', { name: 'Siguiente' }).click();
  // Skip the highlights step; this test is about the scene.
  await dialog.getByRole('button', { name: 'Siguiente' }).click();
  ...
  for (let step = 5; step < 8; step++) await dialog.getByRole('button', { name: 'Siguiente' }).click();
  await dialog.getByRole('button', { name: 'Guardar producto', exact: true }).click();
  await expect(page.getByRole('heading', { name: 'Producto creado', exact: true })).toBeVisible();
  expect(uploads).toBe(1);
