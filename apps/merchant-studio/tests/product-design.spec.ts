...
for (const width of [1280, 390, 320]) test(`colorful product choices and quantity work at ${width}px`, async ({ page }) => {
  ...
  // The only available combination (Original / 340 g) is preselected, so purchase is ready on open.
  await expect(page.getByRole('button', { name: 'Original', exact: true })).toHaveAttribute('aria-pressed', 'true');
  await expect(page.getByRole('button', { name: 'Añadir al pedido', exact: true })).toBeEnabled();
