...
export function createSourceDesignJobs(api: MerchantStudioApi) {
  ...
  return function mount(host: HTMLElement, options: { storeId: string; version: SourceVersion; page: string; locked: boolean; historical: boolean; saved: () => Promise<void> }) {
    ...
    function draw() {
      ...
      // The panel opens toward the preview; right-aligned near the canvas edge it would slide under the chat column.
      const panel = host.querySelector<HTMLElement>('.source-design-job section'), canvas = host.closest('.source-canvas')?.getBoundingClientRect();
      if (panel && canvas && panel.getBoundingClientRect().left < canvas.left + 8) { panel.style.right = 'auto'; panel.style.left = '0'; }
