  ...
  return function mount(host: HTMLElement, options: { storeId: string; version: SourceVersion; page: string; settings: SourceGenerationSettings; locked: boolean; historical: boolean; prepare: (text: string) => void; saved: () => Promise<void> }) {
    ...
    function draw() {
      ...
      const resourceBody = library ? `<p>Define cómo puede usar YAPI cada imagen o video. Se guarda automáticamente, sin créditos de IA.</p><div class="source-asset-list">${library.assets.filter(asset => asset.kind !== 'icon').map((asset, index) => {
        ...
        const mediaLabel = asset.kind === 'video' ? 'Video MP4' : `Imagen ${asset.path.split('.').at(-1)!.toUpperCase()}`;
        ...
        return `<article class="source-asset-row">${preview}<div><strong>${escape(mediaLabel)}</strong><p class="source-asset-name">${escape(asset.description || `${asset.kind === 'video' ? 'Video' : 'Imagen'} ${index + 1}`)}</p><label>Uso<select data-asset-role="${index}" ${disabled || options.historical ? 'disabled' : ''}>${roles.map(([id, label]) => `<option value="${id}" ${value.role === id ? 'selected' : ''}>${label}</option>`).join('')}</select></label><label>Descripción<input data-asset-description="${index}" value="${escape(value.description)}" maxlength="500" ${disabled || options.historical ? 'disabled' : ''}></label><small>${asset.references.length ? `En ${escape(asset.references.join(', '))}` : 'Guardado, todavía no aparece en el sitio'}</small><button type="button" data-use-asset="${escape(asset.path)}" ${disabled || value.role === 'unused' ? 'disabled' : ''}>${asset.kind === 'video' ? 'Usar este video' : 'Usar esta imagen'}</button></div></article>`;
      ...
      host.querySelectorAll<HTMLButtonElement>('[data-use-asset]').forEach(button => button.addEventListener('click', () => {
        ...
        const asset = library?.assets.find(asset => asset.path === path);
        const kind = asset?.kind === 'video' ? 'video' : asset?.kind === 'image' ? 'imagen' : 'icono';
        options.prepare(`Usa ${kind === 'imagen' ? 'la imagen guardada' : `el ${kind} guardado`} ${path} en esta página (${page}), respetando su uso confirmado y el diseño actual.${kind === 'video' ? ' Si se muestra en la página, usa el archivo MP4 como video reproducible, sin sustituirlo por una imagen fija.' : ''}`);
      }));
