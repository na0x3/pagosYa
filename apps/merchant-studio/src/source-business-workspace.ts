...
export async function mountBusinessWorkspace(app: HTMLDivElement) {
    ...
    try {
        ...
        if (kind === 'integrations') {
            ...
            // SKU linking for one connection: drafts survive redraws until saved.
            let linking: { connectionId: string; data: Awaited<ReturnType<MerchantStudioApi['integrationMappings']>>; drafts: Map<string, string>; search: string } | undefined;
            const kindLabels: Record<string, string> = { CUSTOM_DATABASE: 'Base de datos propia', SHOPIFY: 'Shopify', WOOCOMMERCE: 'WooCommerce', POS: 'Punto de venta', ACCOUNTING: 'Contabilidad', OTHER: 'Otro sistema' };
            const rowKey = (productId: string, variantId?: string | null) => `${productId}:${variantId || ''}`;
            const reasonCounts = (skipped: Array<{ reason: string }>) => [...skipped.reduce((counts, item) => counts.set(item.reason, (counts.get(item.reason) || 0) + 1), new Map<string, number>())];
            function inventorySection() {
                const rules = `<ul class="integration-rules"><li><strong>Stock:</strong> tu sistema manda en cada SKU vinculado. pagosYa guarda cada cambio como movimiento de inventario.</li><li><strong>Ventas en pagosYa:</strong> si envías un conteo con <code>asOf</code>, restamos lo vendido después de ese momento, así no se pierden ventas entre sincronizaciones.</li><li><strong>Avisos a tu sistema:</strong> cada venta llega a tus webhooks como <code>inventory.sold</code>, con SKU y cantidad.</li><li><strong>Productos con opciones:</strong> vincula un SKU por combinación (por ejemplo, Negro y Marfil por separado).</li><li><strong>Nombre, precio y fotos:</strong> se editan en pagosYa; la sincronización no los cambia.</li></ul>`;
                const rows = connections.map(item => {
                    const run = item.lastRun, skipped: Array<{ externalSku: string; reason: string }> = run?.details?.skipped || [];
                    const report = run ? run.status === 'FAILED' ? `<p class="integration-report is-error">La última sincronización falló: ${esc(run.errorMessage || 'error desconocido')}</p>` : `<p class="integration-report">Última sincronización: ${run.itemCount} ${run.itemCount === 1 ? 'SKU actualizado' : 'SKU actualizados'}${skipped.length ? ` · ${skipped.length} sin aplicar` : ''}.</p>${skipped.length ? `<details class="integration-skipped"><summary>Ver SKU sin aplicar</summary><ul>${reasonCounts(skipped).map(([reason, count]) => `<li><strong>${esc(reason)}</strong> (${count}): ${esc(skipped.filter(entry => entry.reason === reason).slice(0, 12).map(entry => entry.externalSku).join(', '))}</li>`).join('')}</ul></details>` : ''}` : '';
                    return `<article class="connection-row"><div><strong>${esc(item.name)}</strong><small>${esc(kindLabels[item.kind] || item.kind)} · ${item.mappingCount || 0} ${item.mappingCount === 1 ? 'SKU vinculado' : 'SKU vinculados'} · ${item.lastSyncAt ? `Última sincronización: ${esc(new Date(item.lastSyncAt).toLocaleString('es-BO'))}` : 'Pendiente de la primera sincronización'}</small>${report}</div><div class="connection-actions"><span class="business-badge ${item.lastSyncAt ? 'is-ready' : ''}">${item.lastSyncAt ? 'Sincronizada' : 'Creada'}</span><button type="button" data-link-connection="${esc(item.id)}">${linking?.connectionId === item.id ? 'Cerrar vínculos' : 'Vincular SKU'}</button></div></article>${linking?.connectionId === item.id ? linkingPanel(item.id) : ''}`;
                }).join('') || '<p>Aún no tienes conexiones.</p>';
                return `<section class="business-edit"><h2>Conexiones de inventario</h2><p>Conecta tu sistema de inventario mediante la API de sincronización. Una conexión se muestra como sincronizada después de recibir datos.</p><details class="integration-merge" open><summary>Cómo se combinan los datos</summary>${rules}</details>${rows}<form data-connection-form><div class="business-fields"><label>Nombre<input name="name" maxlength="80" required value="${esc(connectionDraft.name)}" placeholder="Inventario de mi tienda"></label><label>Sistema<select name="kind"><option value="CUSTOM_DATABASE">Base de datos propia</option><option value="SHOPIFY">Shopify mediante API</option><option value="WOOCOMMERCE">WooCommerce mediante API</option><option value="POS">Punto de venta</option><option value="ACCOUNTING">Contabilidad</option><option value="OTHER">Otro sistema</option></select></label></div><button class="business-primary" type="submit">Crear conexión</button></form>${issued ? `<section class="business-note"><strong>Conexión creada</strong><p>Guarda esta clave. Solo se muestra ahora.</p><label>ID<input readonly value="${esc(issued.id)}"></label><label>Clave de sincronización<input readonly value="${esc(issued.secret)}"></label>${apiExample(issued.id)}</section>` : ''}</section>`;
            }
            function apiExample(connectionId: string) {
                const base = new URL(API_BASE_URL, location.href).href.replace(/\/$/, '');
                return `<details class="integration-api"><summary>Ejemplo para tu desarrollador</summary><pre><code>curl -X POST ${esc(base)}/integrations/${esc(connectionId)}/stock \\
  -H "Authorization: Bearer TU_CLAVE" -H "Content-Type: application/json" \\
  -d '{"items":[
    {"externalSku":"LAMP-NEGRO","stock":7,"asOf":"2026-09-15T10:00:00Z"},
    {"externalSku":"LAMP-MARFIL","delta":-2}
  ]}'</code></pre><p class="business-help">Envía <code>stock</code> (conteo total, con <code>asOf</code> opcional) o <code>delta</code> (entradas y salidas). La respuesta lista los SKU aplicados y los que no se pudieron aplicar, con el motivo.</p></details>`;
            }
            function linkingPanel(connectionId: string) {
                if (!linking || linking.connectionId !== connectionId) return '';
                const { data, drafts, search } = linking;
                const mappingFor = (productId: string, variantId?: string | null) => data.mappings.find(mapping => mapping.paymentLinkId === productId && (mapping.variantId || '') === (variantId || ''));
                const rows = data.products.flatMap(product => (product.variants.length ? product.variants : [undefined]).map(variant => ({ product, variant })))
                    .filter(({ product, variant }) => `${product.name} ${variant?.name || ''}`.toLocaleLowerCase().includes(search.toLocaleLowerCase()));
                const linked = data.products.reduce((count, product) => count + (product.variants.length ? product.variants.filter(variant => mappingFor(product.id, variant.id)).length : Number(Boolean(mappingFor(product.id)))), 0);
                const total = data.products.reduce((count, product) => count + Math.max(1, product.variants.length), 0);
                return `<section class="integration-linking" aria-label="Vincular SKU"><header><div><h3>Vincular SKU</h3><p class="business-help">${linked} de ${total} artículos vinculados. Escribe el SKU exacto de tu sistema; déjalo vacío para no sincronizar ese artículo.</p></div><label>Buscar<input type="search" data-link-search value="${esc(search)}" placeholder="Producto o combinación"></label></header><div class="business-table-scroll"><table><thead><tr><th>Producto</th><th>Stock en pagosYa</th><th>SKU en tu sistema</th></tr></thead><tbody>${rows.map(({ product, variant }) => {
                    const key = rowKey(product.id, variant?.id), mapping = mappingFor(product.id, variant?.id), value = drafts.get(key) ?? mapping?.externalSku ?? '';
                    const stock = variant ? variant.stock : product.stock;
                    return `<tr><td><strong>${esc(product.name)}</strong>${variant ? `<small class="integration-combination">${esc(variant.name)}</small>` : ''}</td><td>${stock === null ? 'Sin límite' : stock}</td><td><input data-sku-row="${esc(key)}" value="${esc(value)}" maxlength="120" aria-label="SKU de ${esc(product.name)}${variant ? ` ${esc(variant.name)}` : ''}" autocomplete="off" spellcheck="false"></td></tr>`;
                }).join('') || '<tr><td colspan="3">No hay productos para esta búsqueda.</td></tr>'}</tbody></table></div><div class="business-actions"><button class="business-primary" type="button" data-save-links>Guardar vínculos</button></div>${apiExample(connectionId)}</section>`;
            }
            ...
            function drawIntegrations() {
                ...
                app.querySelectorAll<HTMLButtonElement>('[data-link-connection]').forEach(button => button.onclick = () => {
                    const connectionId = button.dataset.linkConnection!;
                    if (linking?.connectionId === connectionId) { linking = undefined; drawIntegrations(); return; }
                    void run(async () => { linking = { connectionId, data: await api.integrationMappings(storeId, connectionId), drafts: new Map(), search: '' }; }, () => { message = ''; drawIntegrations(); app.querySelector<HTMLElement>('[data-link-search]')?.focus(); });
                });
                app.querySelector<HTMLInputElement>('[data-link-search]')?.addEventListener('input', event => {
                    const input = event.target as HTMLInputElement, position = input.selectionStart;
                    linking!.search = input.value; drawIntegrations();
                    const next = app.querySelector<HTMLInputElement>('[data-link-search]')!; next.focus(); next.setSelectionRange(position, position);
                });
                app.querySelectorAll<HTMLInputElement>('[data-sku-row]').forEach(input => input.addEventListener('input', () => { linking!.drafts.set(input.dataset.skuRow!, input.value); dirty = true; }));
                app.querySelector<HTMLButtonElement>('[data-save-links]')?.addEventListener('click', () => {
                    const current = linking!;
                    let result = '';
                    void run(async () => {
                        let saved = 0, removed = 0;
                        for (const [key, raw] of current.drafts) {
                            const [paymentLinkId, variantId] = key.split(':'), externalSku = raw.trim();
                            const mapping = current.data.mappings.find(item => item.paymentLinkId === paymentLinkId && (item.variantId || '') === variantId);
                            if (!externalSku && mapping) { await api.deleteIntegrationMapping(storeId, current.connectionId, mapping.id); removed++; }
                            else if (externalSku && externalSku !== mapping?.externalSku) { await api.saveIntegrationMapping(storeId, current.connectionId, { paymentLinkId, ...(variantId ? { variantId } : {}), externalSku }); saved++; }
                        }
                        current.data = await api.integrationMappings(storeId, current.connectionId); current.drafts.clear();
                        connections = await api.integrationConnections(storeId); dirty = false;
                        result = saved || removed ? `Guardamos ${saved} ${saved === 1 ? 'vínculo' : 'vínculos'}${removed ? ` y quitamos ${removed}` : ''}.` : 'No había cambios en los vínculos.';
                    }, () => { message = result; drawIntegrations(); });
                });
            }
